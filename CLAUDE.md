...
**Localization (i18n).** Flutter `gen-l10n`: strings live in ARB files under [lib/l10n/](lib/l10n/) (`app_en.arb` is the template, plus `app_fr.arb`, `app_es.arb`); `flutter gen-l10n` (or any build) regenerates `AppLocalizations` (also in `lib/l10n/`, committed). `main.dart` wires `localizationsDelegates`/`supportedLocales` and `locale: AppSettings.instance.locale`, so the Settings language picker (English / Français / Español / System) flips the whole app live. In widgets read strings via `AppLocalizations.of(context).<key>` (non-null getter). **To add a string: add the key to all three ARB files, run `flutter gen-l10n`, then reference it.** Migration is in progress — login, onboarding, dashboard, and the Settings language section are localized; other screens still hold literal English strings and should be migrated key-by-key. Don't hardcode new user-facing copy.
